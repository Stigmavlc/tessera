...
- Table drags always carry exactly one tile, regardless of hold duration. Automatic and long-hold run-tail pickup were removed after repeated accidental group selection. Do not reintroduce implicit batch selection on the board; rack batch selection remains unchanged.
...
### Pro-player feedback follow-up (local)

`extendMeldAtEnd` also accepts compatible two-tile drafts (different-colour equal numbers, consecutive same-colour values, or a joker); `analyzeMeld` still rejects committing fewer than three. `approachedMeld` supplies the same narrow approach area to dragging and tap-on-felt placement, with opening eligibility checked in the UI. Tile colours now use a brighter yellow with a fine darker number edge, saturated red/blue, and deep black. Verified 67 tests and production build, plus touch reproduction of separate tens, individual seven extraction, deliberate run hold, and Take back.

### 2026-09-09 placement and crowded-table update

`isCompatibleMeldDraft` gates new rack batches and destination joins from either rack or board. Singles and compatible pairs are allowed; duplicate identities, duplicate same-colour numbers, mixed-colour runs and unfilled gaps are refused without mutating state or Undo history. Keep removal permissive: extracting a tile can leave an incomplete source that the player repairs before committing. Deliberate duplicate-run splitting remains supported because it produces separate groups.

`BoardDropZone` now wraps a measured/clipped `.board-stage` and a sibling `.board-toolbar` in `.board-area`. Camera, audio and recovery controls live outside the stage; do not overlay them on playable felt. Landscape grid placement belongs to `.board-area`; camera measurements and drop targeting still use the inner stage. Fit uses `stageRef` because its button is now a sibling.
