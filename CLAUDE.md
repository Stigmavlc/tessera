...
- **`src/audio.ts`** — all sound: a synthesized Web Audio turn-timer tick (last 10s, volume ramps with urgency) and a looping music player for `public/audio/lobby.mp3` / `table.mp3` (crossfade on screen change, autoplay-safe gesture re-kick, silent no-op when files are missing). Gated by the persisted "Table sounds" setting (`tessera.sound`; `tessera.haptics` likewise), which lives in `App` and flows into `GameScreen` as props.
